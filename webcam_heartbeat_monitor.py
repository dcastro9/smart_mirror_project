...
class WebcamHeartbeatMonitor(object):
    ...
    def debug(self): #for debugging purposes
        ...
        for val in range(180):
            ...
            cv2.waitKey(20)
        #tmp.destory
        evm = EulerianVideoMagnification(frames, levels=1)
    ...
    def run(self):
        while self._rval:
            ...
            if (face_image != None):
                ...
                self._img_queue.empty()
            # If the queue is larger than the requested num_frames:
...
class FaceDetector(object):
    ...
    def process(self, img):
        ...
        for x1, y1, x2, y2 in rects:
            if ((round((y2-y1)/(self._dimension[0])))>0 and
                int(round((x2-x1)/(self._dimension[1])))>0):
